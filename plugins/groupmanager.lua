...
local lang = redis:get(hash)
 ...
 if not lang then
  ...
  return '_گروه با موفیت از لیست گروه های مدیریتی ربات حذف شد._\n_توسط=>_*[*@'..check_markdown(msg.from.username or '')..'*]*'
end
end
...
if not lang then
 ...
 return "`شما مدیر گروه نمیباشید`\n*این دستور مخصوص مدیران و معاونان گروه می باشد.*\nبرای خرید گروه به آیدی زیر مراجعه کنید\n@TAHA_RI "
end
end
...
if not lang then
 ...
 return "_ارسال لینک در گروه ممنوع شد._"
end
end
end
...
elseif lang then
return "_ارسال لینک در گروه ممنوع نمیباشد._"
end
else 
...
local settings = data[tostring(target)]["settings"] 
 text = "*تنظیمات گروه:*\n▫️قفل ویرایش پیام : *"..settings.lock_edit.."*\n▫️قفل لینک : *"..settings.lock_link.."*\n▫️قفل ورود : *"..settings.lock_join.."*\n▫️قفل تگ : *"..settings.lock_tag.."*\n▫️قفل پیام مکرر : *"..settings.flood.."*\n▫️قفل هرزنامه : *"..settings.lock_spam.."*\n▫️قفل فراخوانی : *"..settings.lock_mention.."*\n▫️قفل عربی : *"..settings.lock_arabic.."*\n▫️قفل صفحات وب : *"..settings.lock_webpage.."*\n▫️قفل فونت : *"..settings.lock_markdown.."*\n▫️پیام خوشآمد گویی : *"..settings.welcome.."*\n▫️قفل سنجاق کردن : *"..settings.lock_pin.."*\n▫️محافظت در برابر ربات ها : *"..settings.lock_bots.."*\nحداکثر پیام مکرر : *"..NUM_MSG_MAX.."*\nحداکثر حروف مجاز : *"..SETCHAR.."*\n▫️زمان بررسی پیام های مکرر : *"..TIME_CHECK.."*\n▫️تاریخ انقضا : *"..expire_date.."*\n▫️زبان سوپرگروه : *FA*"
end
return text
end
--------Mutes---------
...
local function getpro(arg, data)
   if data.photos_[0] then
       ...
       elseif lang then
      tdcli.sendMessage(msg.to.id, msg.id_, 1, "_شناسه گروه :_ `"..msg.to.id.."`\n_شناسه شما :_ `"..msg.from.id.."`\n", 1, 'md')
     end
   else
       if not lang then
...
return "`زبان گروه تنظیم شد`\n"..msg_caption
 ...
 redis:del(hash)
return "`زبان گروه تنظیم شد`"..msg_caption
end
end
...
_شما میتوانید از [!/#] در اول دستورات برای اجرای آنها بهره بگیرید🔺_
...
if not lang then
text = [[
*دستورات مدیریت گروه مانستر تی جی:*
	...
	local secchk = true
		...
		if secchk then
			...
			if not lang then
				...
				return tdcli.sendMessage(msg.to.id, 0, 1, '`لطفا برای کارکرد کامل دستورات، ربات را به مدیر گروه ارتقا دهید.`', 1, "md")
			end
		end
	end, nil)
 end
